"""GD0 verification: stream test_audio.pcm over /ws (vi->en, phowhisper) against
the running server (LOG_OBS=1) and print received events. The server's own
stdout (separate process) is where [CFG]/[SEG]/[RES] lines appear.
"""
import asyncio
import json
import sys
...
async def main():
    ...
    async with websockets.connect(WS_URL, max_size=None) as ws:
        await ws.send(json.dumps({"type": "config", "src": "vi", "tgt": "en", "backend": "phowhisper"}))
