"""
Streaming Vietnamese ASR via faster-whisper CTranslate2 + sliding window.

Default model: erax-ai/EraX-WoW-Turbo-V1.1-CT2
  Whisper Large-v3 Turbo fine-tuned on 8 Vietnamese dialects + 10 other languages.
  CTranslate2 int8 — ~350 ms for 30 s audio on GPU.
...
SAMPLE_RATE   = 16000
TICK_S        = 0.40   # re-encode every 400 ms (Whisper is heavier than RNNT)
...
_MODEL_NAME = "erax-ai/EraX-WoW-Turbo-V1.1-CT2"
_LOCAL_NAME = "EraX-WoW-Turbo-V1.1-CT2"
...
def load_model(model_path: str, **_kwargs):
    ...
    print(f"[Whisper] loading {Path(model_path).name} on {device} ({compute_type}) ...", flush=True)
    try:
        model = WhisperModel(model_path, device=device, compute_type=compute_type)
    except Exception as e:
        if device != "cpu":
            print(f"[Whisper] GPU failed ({e}), retrying on CPU ...", flush=True)
...
class StreamingASR:
    """
    Sliding-window ASR using faster-whisper (CTranslate2).

    on_update(committed, interim) contract:
    ...
    def _loop(self) -> None:
        while not self._stop.is_set():
            ...
            try:
                ...
                print(f"[Whisper] tick error: {e}", flush=True)
    ...
    def _tick(self) -> None:
        ...
        if not text or not _VI_RE.search(text):
            return   # empty or English hallucination
        ...
        if text != self._last_text:
            ...
            print(f"[Whisper] {ms:.0f}ms buf={n/SAMPLE_RATE:.2f}s -> {repr(text[:60])}", flush=True)
    ...
    def _commit(self) -> None:
        ...
        if text:
            with self._lock:
                self._committed = (self._committed + " " + text).strip()
                committed = self._committed
            print(f"[Whisper] commit: {repr(committed[-80:])}", flush=True)
            self.on_update(committed, "")
        else:
            with self._lock:
                committed = self._committed
            self.on_update(committed, "")
